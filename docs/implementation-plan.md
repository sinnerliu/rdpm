...
### 阶段一：工程基础与核心数据模型
- [ ] **任务 1.1**：创建根目录 `Cargo.toml` 工作区配置与依赖项版本声明。
- [ ] **任务 1.2**：在 `crates/rdpm_core` 中实现领域模型与便携数据存储：
...
- [ ] **任务 1.3**：在 `crates/rdpm_vault` 中实现 Windows DPAPI 凭据安全保管箱：
...
- [ ] **任务 2.1**：在 `crates/rdpm_rdp_host` 中编写 C++/ATL 宿主底层 Shim：
...
- [ ] **任务 2.2**：编写 `build.rs` 脚本，使用 `cc` crate 在 MSVC 环境下自动编译 C++ Shim 与 ATL 依赖。
- [ ] **任务 2.3**：在 Rust 侧实现安全封装与生命周期管理（`RdpHostHandle`、事件通知回调通道）。

### 阶段三：GPUI 界面与多 Tab 容器集成
- [ ] **任务 3.1**：初始化 GPUI 应用程序骨架与主窗口。
- [ ] **任务 3.2**：实现左侧服务器树视图（`ServerTreeView`）：
  - 树形折叠展开与节点渲染；
  - 顶部快速搜索/拼音过滤框；
  - 双击触发连接事件、右键上下文菜单（添加、编辑、删除服务器/分组）。
- [ ] **任务 3.3**：实现多 Tab 标签页容器（`TabContainerView`）：
  - 支持多 Tab 动态增删、激活指示器与关闭按钮；
  - 实现 Airspace 协同算法：在 GPUI 布局后精准计算并同步 Native Child HWND 的物理像素尺寸与位置；
  - Tab 切换时的可见性（Show/Hide）控制与焦点转交机制。

### 阶段四：会话状态机与断线自动重连
- [ ] **任务 4.1**：在 `crates/rdpm_session` 中实现会话状态机（未连接、连接中、已连接、重连中、已断开）。
- [ ] **任务 4.2**：处理断线原因判定，过滤用户主动关闭与异常掉线。
- [ ] **任务 4.3**：实现指数退避重连算法（1s -> 2s -> 4s -> 8s -> 15s），并在 Tab 和状态栏上呈现动态重连进度条与旋转动画。

### 阶段五：Workspace 保存与一键恢复运维环境
- [ ] **任务 5.1**：在 `crates/rdpm_workspace` 中实现当前工作区状态采集器（捕获当前打开的全部 Server ID、排序及当前激活项）。
- [ ] **任务 5.2**：实现工作区存盘管理（多工作区配置切换、设为默认工作区、退出自动保存）。
- [ ] **任务 5.3**：实现阶梯式并发恢复调度器（Staggered Queue）：
  - 优先秒级连接恢复上次活动的 Tab；
